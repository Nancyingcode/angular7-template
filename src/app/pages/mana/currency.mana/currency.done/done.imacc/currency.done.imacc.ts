...
const { currencyDoneImaccDetail } = Config.userMana;
...
@Component({
  selector: 'app-currency-done-imacc',
  templateUrl: './currency.done.imacc.html',
  styleUrls: ['./currency.done.imacc.less']
})

export class CurrencyDoneImaccComponent implements OnInit {
  ...
  detail = ({ orderId }) => {
    this.router.navigate([currencyDoneImaccDetail, { orderId }]);
  }
}
