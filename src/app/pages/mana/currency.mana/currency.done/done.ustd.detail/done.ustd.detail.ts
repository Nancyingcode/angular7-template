...
const console = new Log('CurrencyDoneUstdDetailComponent');

@Component({
  selector: 'app-currency-doneusdt-detail',
  templateUrl: './done.ustd.detail.html',
  styleUrls: ['./done.ustd.detail.less']
})

export class CurrencyDoneUstdDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    try {
      this.setData({ flowId: this.orderId, type: 1 });
    } catch (e) {
