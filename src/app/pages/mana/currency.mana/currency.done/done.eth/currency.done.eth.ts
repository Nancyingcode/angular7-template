...
const { currencyDoneEthDetail } = Config.userMana;
const console = new Log('CurrencyDoneEthComponent');
const {
  page,
  pageSize,
  downloadUrl,
...
@Component({
  selector: 'app-currency-done-eth',
  templateUrl: './currency.done.eth.html',
  styleUrls: ['./currency.done.eth.less']
})

export class CurrencyDoneEthComponent implements OnInit {
  ...
  async getData(data: any): Promise<any> {
    const res = await this.cs.getDoneList(data);
    return res;
  }
  ...
  detail = ({ orderId }) => {
    this.router.navigate([currencyDoneEthDetail, { orderId }]);
  }
}
