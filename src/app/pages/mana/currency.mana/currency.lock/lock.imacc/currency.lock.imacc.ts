...
const { currencyLockImaccDetail } = Config.userMana;
...
@Component({
  selector: 'app-currency-lock-imacc',
  templateUrl: './currency.lock.imacc.html',
  styleUrls: ['./currency.lock.imacc.less']
})

export class CurrencyLockImaccComponent implements OnInit {
  ...
  async getData(data: any): Promise<any> {
    const res = await this.cs.getLockList(data);
    return res;
  }
  ...
  detail = ({ orderId }: any): void => {
    this.router.navigate([currencyLockImaccDetail, { orderId }]);
  }
}
