...
const { currencyLockEthDetail } = Config.userMana;
...
@Component({
  selector: 'app-currency-lock-eth',
  templateUrl: './currency.lock.eth.html',
  styleUrls: ['./currency.lock.eth.less']
})

export class CurrencyLockEthComponent implements OnInit {
  ...
  detail = ({ orderId }) => {
    this.router.navigate([currencyLockEthDetail, { orderId }]);
  }
}
