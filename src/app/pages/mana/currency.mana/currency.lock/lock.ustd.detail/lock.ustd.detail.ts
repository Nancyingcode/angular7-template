...
const console = new Log('CurrencyLockUstdDetailComponent');

@Component({
  selector: 'app-currency-lockusdt-detail',
  templateUrl: './lock.ustd.detail.html',
  styleUrls: ['./lock.ustd.detail.less']
})

export class CurrencyLockUstdDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    try {
      this.setData({ flowId: this.orderId, type: 2 });
    } catch (e) {
